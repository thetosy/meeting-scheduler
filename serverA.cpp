...
struct addrinfo hints, *servinfo, *servinfo_serverM, *p, *p_serverM;
int rv;
int numbytes;
...
unordered_map<string, timeslots*> dataA;
...
int main(void){
    if(createServerSocket() != TRUE){
        fprintf(stderr, "Error creating serverA socket\n");
        exit(1);
    }
    fprintf(stdout, "ServerA is up and running using UDP on port %s.\n", MYPORT);
    readBuildData();
    if(sendDataToMainServer() != TRUE){
        exit(1);
    }
    fprintf(stdout, "ServerA finished sending list of usernames to Main Server.\n");
//    timeslots *p = dataA["khloe"];
//    fprintf(stdout, "%d\n", p->times[0].start);
    ...
    while(1){
        ...
        new_buf[numbytes] = '\0';
        fprintf(stdout, "Server A received the usernames from Main Server using UDP over port %s.\n", MYPORT);
        //a_timeslot *timeslot = (a_timeslot*)malloc(sizeof(a_timeslot));
        a_timeslot timeslot;
        timeslot.count = 0;
        findCommonTimeSlots(new_buf, dataA, &timeslot);
        //printf(stdout, "here2\n");
        ...
        if((numbytes = sendto(send_sockfd, reply, strlen(reply), 0, p_serverM->ai_addr, p_serverM->ai_addrlen)) == -1){
            perror("serverA: send intersection");
            exit(1);
        }
        fprintf(stdout, "Server A finished sending the response to Main Server\n");
    }
